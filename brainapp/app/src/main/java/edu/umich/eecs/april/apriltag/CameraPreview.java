...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private static String TAG = "AprilTag";
    /**
     ...
     * required {@link #}'s parameters and starts the preview stream.
     *
     * @param holder
     */
    @Override
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        if (this.surfaceHolder.getSurface() == null) {
            Log.e(TAG, "surfaceChanged(): surfaceHolder is null, nothing to do.");
            return;
        }
    ...
    private void updateCameraDisplayOrientation() {
        CameraActivity parent = (CameraActivity)this.getContext();
        Camera camera = parent.getCamera();
        int cameraID = parent.getCameraID();

        if (camera == null) {
            Log.e(TAG, "updateCameraDisplayOrientation(): warning, camera is null");
            return;
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.i(TAG, "onMeasure(): set surface dimension to " + width + "x" + height);
    }
    ...
    private void setupCamera() {
        ...
        if (camera == null) {
            Log.e(TAG, "setupCamera(): warning, camera is null");
            return;
        }
        ...
        Log.d(TAG, "setupCamera(): settings applied:\n\t"
        ...
        try {
            ...
            Log.e(TAG, "setupCamera(): error setting camera callback.", e);
        }
    }
...
//                cam.addCallbackBuffer(previewBuffer);
    ...
    private synchronized void startCameraPreview(SurfaceHolder holder) {
        ...
        try {
            ...
            Log.e(TAG, "startCameraPreview(): error starting camera preview", e);
        }
    }
